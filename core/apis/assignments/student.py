...
def upsert_assignment(p, incoming_payload):
    ...
    # if incoming_payload.content is "":
    #     return {}, 400
...
def submit_assignment(p, incoming_payload):
    ...
    prev_assignment = Assignment.get_assignment_by_id(submit_assignment_payload.id)

    if prev_assignment.state == 'SUBMITTED':
        return make_response(jsonify(error="FyleError", message='only a draft assignment can be submitted'), 400)
    #
    # prev_assignment_dump = AssignmentSchema().dump(prev_assignment)
    # # print("hhhherrrrrrrr")
    # # print(prev_assignment_dump)
    # with open("myfile.txt","w") as f:
    #     f.write(str(prev_assignment.id))
